...
for numRun in range(50):
    ...
    loss_ = []
    initial_t = torch.ones(n_cate, 1)/n_cate
    t = V(initial_t, requires_grad=False)
    t = t.cuda()
    num_iter = 0
    for MaxIter, step_size in zip(Iter, ss):
        ...
        loss_prev = V(loss.data, requires_grad=False)
        ...
        for i in range(MaxIter):
            ...
            # t = t + step_size*(2*(loss - loss.mean()) - (loss_prev - loss_prev.mean()))
            t = t + step_size*(2*loss - loss_prev)
            t = utils.simplex_projection(t, n_cate)
            t = V(t, requires_grad=False)
...
save_file_name = '{:s}'.format('SA-MGDA') + '_' + '_'.join(Iter_str) + '_' + '_'.join(ss_str)
